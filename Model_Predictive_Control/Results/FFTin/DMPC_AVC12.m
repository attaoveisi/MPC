...
% Eq. (12)
bode(ss(W,Hs,V,0))
sys_out1 = c2d(ss(W,Hs,V,0),dt,'zoh');
S = sys_out1.A;
hold on
bode(sys_out1,ww)
...
% Qf(1:2,1:2)=(1e1)*Qf(1:2,1:2);
% Qf(3:4,3:4)=(1e1)*Qf(3:4,3:4);
% Qf(5:6,5:6)=(1e1)*Qf(5:6,5:6);
Rf=(2e-1)*eye(size(C*C'));
Pf=(1e-8)*eye(size(A*A'));
% Pf(1:2,1:2)=(1e0)*Pf(1:2,1:2);
% Pf(3:4,3:4)=(1e0)*Pf(3:4,3:4);
% Pf(5:6,5:6)=(1e0)*Pf(5:6,5:6);
...
[eig(A) eig(A-Lf(:,1)*C) eig(A-L2(:,1)*C)]
% [eig(A) eig(A-L2(:,1)*C)]
...
%%
sysk = ss(A_nom,[B_nom H_nom],C_nom,0);

[kest,L,P] = kalman(sysk,5e4,1,0);

%%%%%%% Optimal Gain
syst = ss(A_nom,B_nom,C_nom,0);
[K,S,e] = lqry(syst,1e6,1e0*eye(2),0);
