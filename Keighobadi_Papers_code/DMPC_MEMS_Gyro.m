...
%w1=7;
x0=0.1;
%State Space Model ( Nominal seytem line 27-30)
A_con=[zeros(2,2) eye(2);-K -D];
B_con=[zeros(2,2);eye(2)];
Cm=[1 0 0 0;0 1 0 0];

%% Discretization of nominal system (3 lines above)
...
% laguerre poles for each control inputs
a1=0.5; a2=0.5;

% laguere number of components (n) in Eq. (20)
N1=5; N2=5;

% stability circle (prescribed stability LQR book of australian)
lambda=0.90;


[K_lqr,P_inf,E]=dlqr(A/lambda,B/lambda,Q,R);

% like the book Eq. (4.&) Liuping wang 
alpha=2.1;
Gam=lambda/alpha;
...
% initial condition of gyroscope
...
% Kalman filter for augmented system
